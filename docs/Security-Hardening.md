...
**NVD database caching.** `backend/pom.xml`'s `dependency-check-maven` plugin pins a fixed `dataDirectory` (`~/.dependency-check-data`) and `nvdValidForHours` (24h), and `.github/workflows/backend.yml` caches that directory across runs (`actions/cache`, keyed per run with a prefix `restore-keys` fallback to the most recent prior run). Without this, every run re-downloaded the entire NVD CVE database from scratch, which - combined with NVD's public, no-API-key rate limit - could turn the step into a multi-hour, eventually-failing job. An optional `NVD_API_KEY` repository secret (free, see [nvd.nist.gov/developers/request-an-api-key](https://nvd.nist.gov/developers/request-an-api-key)) is wired through if one is ever added, raising the rate limit further; its absence is the plugin's normal, supported unauthenticated mode.
