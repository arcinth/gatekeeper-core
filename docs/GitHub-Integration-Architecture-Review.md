...
# 8. POST-REVIEW UPDATE: Milestone 8 — Repository Onboarding (Implemented)

This review's Section 0 claimed GitHub check-run creation was unimplemented; that has since shipped (the Verdict-driven "GateKeeper" check and the separate "GateKeeper Review" check for reviewer decisions both exist and are wired via `VerdictProducedEvent`/`ReviewDecisionRecordedEvent`, following exactly the shape Section 5/7 above proposed for Phase 4). That superseded claim is left in place above rather than rewritten, since this document is a point-in-time review, not a living spec - this section is the living addendum.

**What Milestone 8 actually closed:** not a gap in the webhook pipeline (which this review found solid), but a gap in *reachability* - `github_installations`/`repositories` rows were only ever visible through direct database inspection or manual `curl`. There was no button, no installation list, no way for a Platform Engineer to start or observe the connection process from inside the product at all. Manual repository creation (`POST /api/v1/repositories`) existed as an alternate path but was unreachable from the shipped frontend and, more importantly, structurally dead: a manually-created repository has no `githubRepositoryId`/`githubInstallation`, and `RepositoryLookupService.findLinkedRepository` requires both to resolve a `pull_request` webhook - such a repository could never receive real analysis. Milestone 8 removed that path rather than extend it.

**What was added, reusing the existing pipeline exactly as built (no redesign):**
- `GitHubInstallationController` (`GET /install-url`, `GET /installations`, `GET /installations/{id}`, `POST /installations/{id}/sync`) - a pure read/trigger layer over `GitHubInstallation`, described in full in [API-Design.md](./API-Design.md)'s GitHub Installations API section.
- `GitHubInstallationStatus` (`CONNECTING`/`SYNCING`/`ACTIVE`/`ERROR`/`DISCONNECTED`) - a UI-facing synchronization-health status, distinct from the pre-existing `active` boolean (which only ever answered "does this installation still exist on GitHub"). Maintained by three new `GitHubInstallationService` methods (`markSyncing`/`markSynced`/`markSyncFailed`) that `GitHubRepositorySyncService.synchronize` now calls around its own unchanged external-API call, plus `lastSuccessfulSyncAt`/`lastSyncError` columns so a sync failure is visible in the UI instead of silently leaving repositories stale.
- `GitHubRepositorySyncService.synchronize` made `public` - the exact same method the `installation` webhook's async listener already called, now also called synchronously by the sync endpoint above, so the onboarding callback page and a manual "Resync now" action get an immediate result instead of waiting on the async event round trip. No parallel implementation.
- A frontend "GitHub Connections" section on the Repositories page (not a new nav item - onboarding belongs inside Repository Management) plus a `/repositories/connect/callback` page, which is where GitHub's own installation flow redirects the browser after the user completes it on github.com.
- `RepositoryService.link`/`unlink` (the webhook-driven repository upsert/deactivate path, unchanged in its actual linking logic) now record `REPOSITORY_CONNECTED`/`REPOSITORY_UPDATED` audit events - previously only manual creation was audited, so the only path that ever actually worked was, until now, invisible to the audit trail.

**Deliberately not done:** no `connectionSource` column (every repository now originates from exactly one source - GitHub App installation - so a field with one possible value was deferred until a second SCM provider is real, not spent on a hypothetical one); no change to webhook signature verification, installation token caching, the analysis pipeline, or the Policy/Security engines.
